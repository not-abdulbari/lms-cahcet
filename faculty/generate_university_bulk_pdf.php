...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($selected_students as $roll_no) {
        ...
        while ($mark = $marks_result->fetch_assoc()) {
            if (!isset($unique_marks[$mark['subject_code']])) {
                $unique_marks[$mark['subject_code']] = $mark;
            }
        }
        ...
        $pdf->SetFont('Times', 'B', 12);
        $pdf->Cell(35, 10, 'Register No.: ', 0, 0, 'R');
        $pdf->SetFont('Times', '', 12);
        $pdf->Cell(50, 10, $student['reg_no'], 0, 1, 'L');
        $pdf->SetFont('Times', 'B', 12);
        $pdf->Cell(25, 10, 'Year: ', 0, 0, 'L');
        $pdf->SetFont('Times', '', 12);
        $pdf->Cell(70, 10, $year, 0, 1, 'L');
        ...
        $pdf->SetFont('Times', '', 12);
        foreach ($unique_marks as $mark) {
            $result = in_array(strtoupper($mark['grade']), ['U', 'UA']) ? 'RA' : 'PASS';
            $pdf->Cell(20, 10, $mark['semester'], 1, 0, 'C');
            $pdf->Cell(40, 10, $mark['subject_code'], 1, 0, 'C');
            $pdf->Cell(80, 10, $mark['subject_name'], 1, 0, 'L');
    ...
    $pdf->Output('Bulk_Student_Reports.pdf', 'D');
} else {
